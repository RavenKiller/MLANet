...
for ep_id  in range(13):
    ...
    steps = sorted([int(v.split(".")[0]) for v in os.listdir("data/vlntj-ce-extend-val/{}/rgb".format(ep_id))])
    pred_action_close = []
    for step_id in steps:
        tic = time.time()
        with open("data/vlntj-ce-extend-val/{}/rgb/{}.png".format(ep_id, step_id), "rb") as f:
            jpg_data = b64encode(f.read())
        with open("data/vlntj-ce-extend-val/{}/depth/{}.png".format(ep_id, step_id), "rb") as f:
            depth_data = b64encode(f.read())
        # print("==========")
        time_info_encode = time.time()-tic
        tic = time.time()
        res = requests.post("http://{}/predict_action".format(SERVER_STR), data={"ep_id":ep_id, "rgb":jpg_data})
        # print(ep_id, step_id)
        time_info_prediction = time.time()-tic

        time_info = json.loads(res.text)["time_info"]
        time_info["encode_b64"] = time_info_encode
        time_info["network_latency"] = time_info_prediction-time_info["decode_b64"]-time_info["forward"]-time_info["observation"]
    ...
    os.makedirs("data/vlntj-ce-extend-val-new/{}/pred_action_close".format(ep_id), exist_ok=True)
    with open("data/vlntj-ce-extend-val-new/{}/pred_action_close/pred_action_close.json".format(ep_id),"w") as f:
