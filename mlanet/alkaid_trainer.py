...
def load_depth_model():
    ...
    model = torch.hub.load("/share/home/tj90055/.cache/torch/hub/isl-org_ZoeDepth_main", "ZoeD_NK", source="local", pretrained=False)
    model.to("cuda")
    model.eval()
    return model
...
class AlkaidEnv:
    ...
    def get_current_obs(self, ep_id, rgb, depth=None, gt_sub=False):
        ...
        if gt_sub:
            ...
            sub_data = instruction_cut_clip([{"instruction_text": inst}], refine=True, append_dot=False, keep_subs=True)
        ...
        }], {"rgb": time_rgb, "depth": time_depth, "inst": time_inst, "sub": time_sub}
...
class AlkaidTrainer(BaseVLNCETrainer):
    ...
    def predict_action(self):
        ...
        real_observations, time_info_detail = self.real_env.get_current_obs(ep_id, rgb, depth)
